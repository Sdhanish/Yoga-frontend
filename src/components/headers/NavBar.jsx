...
const NavBar = () => {
  ...
  useEffect(() => {
    if (scrollPosition > 100) {
      if (isHome) {
        setNavBg(
          "bg-white backdrop-filter backdrop-blur-xl bg-opacity-0 dark:bg-black dark:text-white"
        );
      } else {
        setNavBg("bg-white dark:bg-black dark:text-white");
      }
    } else {
      setNavBg(
        `${
  ...
  return (
    <motion.nav
      className={`${
        isHome ? navBg : "bg-white dark:bg-black backdrop-blur-2xl dark:text-white"
      } ${
            ...
            <div className="flex text-primary">
              <ul className="ml-10 flex items-center space-x-4 pr-4">
                ...
                {user && (
                  <li>
                    <NavLink
                      className="text-base font-bold px-2 py-2 bg-secondary text-green-900 rounded-md"
